...
from dino_runner.utils.constants import BG1, MOON, CLOUD, CLOUD2, GROUND, ICON, DEAD_GOBLIN, SCREEN_HEIGHT, SCREEN_WIDTH, TITLE, FPS, DEFAULT_TYPE
from dino_runner.utils.text_utils import draw_message_component
from dino_runner.utils.text_utils import draw_message_component2
from dino_runner.utils.text_utils import draw_message_component3
...
class Game:
    def __init__(self):
        ...
        self.score = 0
        self.death_count = 0
        self.bg_music = pygame.mixer.Sound('dino_runner/assets/Sounds/BackgroundMusic.ogg')
        self.game_over_music = pygame.mixer.Sound('dino_runner/assets/Sounds/GameOver.ogg')
        self.menu_music = pygame.mixer.Sound('dino_runner/assets/Sounds/MenuMusic.ogg')
        self.beep = pygame.mixer.Sound('dino_runner/assets/Sounds/Beep.ogg')
    ...
    def draw_background(self):
        ...
        self.screen.blit(BG1, (self.x_pos_bg, self.y_pos_bg))
        self.screen.blit(BG1, (image_width + self.x_pos_bg, self.y_pos_bg))
        self.screen.blit(BG1, (image_width + self.x_pos_bg, self.y_pos_bg + 150))
        self.screen.blit(BG1, (image_width + self.x_pos_bg - 450, self.y_pos_bg + 150))
        self.screen.blit(BG1, (image_width + self.x_pos_bg - 650, self.y_pos_bg + 150))
        if self.x_pos_bg >= -image_width:
            self.screen.blit(BG1, (image_width + self.x_pos_bg + 1000, self.y_pos_bg))
            self.x_pos_bg = 0
    ...
    def show_menu(self):
        ...
        if self.death_count == 0:
            ...
            self.screen.blit(BG1, (self.x_pos_bg, self.y_pos_bg))
            self.screen.blit(BG1, (image_width + self.x_pos_bg, self.y_pos_bg))
            self.screen.blit(BG1, (image_width + self.x_pos_bg - 20, self.y_pos_bg + 150))
            self.screen.blit(BG1, (image_width + self.x_pos_bg - 80, self.y_pos_bg + 150))
            self.screen.blit(BG1, (image_width + self.x_pos_bg - 650, self.y_pos_bg + 150))
            self.screen.blit(BG1, (image_width + self.x_pos_bg - 20, self.y_pos_bg + 300))
            self.screen.blit(BG1, (image_width + self.x_pos_bg - 80, self.y_pos_bg + 300))
            self.screen.blit(BG1, (image_width + self.x_pos_bg - 650, self.y_pos_bg + 300))
            self.menu_music.play()
            self.menu_music.set_volume(0.5)
            draw_message_component2("Press any key to start", self.screen)
        else: 
            self.bg_music.stop()
            self.menu_music.stop()
            self.game_over_music.play()
            ...
            draw_message_component( f"Your Score: {self.score}", self.screen, pos_y_center=half_screen_height - 150)
            draw_message_component(f"Your high score: {self.high_score}", self.screen, pos_y_center=half_screen_height - 125)
            draw_message_component( f"Death count: {self.death_count}", self.screen, pos_y_center=half_screen_height - 100)
            if self.score > self.high_score:
                self.high_score = self.score
                with open('HighScore.txt', 'w') as file:
                    file.write(str(self.high_score))

            self.screen.blit(DEAD_GOBLIN, (half_screen_widht - 150, half_screen_height - 150))
